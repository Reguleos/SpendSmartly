...
if (isset($_SESSION['user_id']) && isset($_SESSION['username'])) {
     ...
     $conn = mysqli_connect($sname, $uname, $password, $db_name);
          if (!$conn) {
               ...
               $record_year = $_GET['year'];
               $_SESSION['type'] = $record_type;
               $_SESSION['year'] = $record_year; 
               $record_sql = "SELECT `month`, `amount` FROM `records` WHERE `user_id` = '$user_id' AND `year` = '$record_year' AND `type` = '$record_type';";
               $record_result = mysqli_query($conn, $record_sql);
               while($row = mysqli_fetch_array($record_result)){
                    $record_month[] = $row['month']; 
                    $record_amount[] = $row['amount'];
               }

               $link = "&type=$record_type&year=$record_type";

               if($record_amount[0]==0) {$january   = null;}  else {$january   = $record_amount[0];}
               if($record_amount[1]==0) {$february  = null;}  else {$february  = $record_amount[1];}
               if($record_amount[2]==0) {$march     = null;}  else {$march     = $record_amount[2];}
               if($record_amount[3]==0) {$april     = null;}  else {$april     = $record_amount[3];}
               if($record_amount[4]==0) {$may       = null;}  else {$may       = $record_amount[4];}
               if($record_amount[5]==0) {$june      = null;}  else {$june      = $record_amount[5];}
               if($record_amount[6]==0) {$july      = null;}  else {$july      = $record_amount[6];}
               if($record_amount[7]==0) {$august    = null;}  else {$august    = $record_amount[7];} 
               if($record_amount[8]==0) {$september = null;}  else {$september = $record_amount[8];}
               if($record_amount[9]==0) {$october   = null;}  else {$october   = $record_amount[9];}
               if($record_amount[10]==0) {$november = null;}  else {$november  = $record_amount[10];}
               if($record_amount[11]==0) {$december = null;}  else {$december  = $record_amount[11];}

               $notes_sql = "SELECT `note` FROM `notes` WHERE `user_id`='$user_id' AND `type`='$record_type' AND `year`='$record_year';";
...
<body onload="notesFunction()">
     ...
     <div class="body"> 
          ...
          <?php } ?>
          <div class="chart">
               <form action="Edit-Records-Actions.php" method="post" id="records-form">
